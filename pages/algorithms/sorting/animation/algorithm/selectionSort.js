export function SelectionSort(contextBackground, contextSwap, barsArray, speed, paused) {
    ...
    console.log("Selection Sort",contextBackground, contextSwap, barsArray, speed)
        ...
        for (let i = 0; i < length - 1; ++i){
            ...
            setTimeout(()=>{
                for (j = i + 1; j < length; ++j){
                    if (barsArray[j].height < barsArray[min].height)
                        min = j;
                }
                setTimeout(()=>{
                    swapAnimation(barsArray[i], barsArray[min])
                }, j * (1000/speed));
            }, i * (length) * (1000/speed) * 2);
        }
    }
    ...
    function clearBackground(ctx, bar1, bar2){
        ctx.clearRect(bar1.x - 10, 0, 40, ctx.canvas.height)
        ctx.clearRect(bar2.x - 10, 0, 40, ctx.canvas.height)
    }
    ...
    function swapAnimationFrame(ctx, bar1, bar2, i) {
        ctx.clearRect(bar1.x - 1, 0, bar2.x-bar1.x+30, ctx.canvas.height)
