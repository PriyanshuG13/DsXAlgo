export function InsertionSort(contextBackground, contextSwap, barsArray, speed) {
    ...
    console.log("Insertion Sort",contextBackground, contextSwap, barsArray, speed)

    function doInsertionSort(){
        for (let i = 1; i < length; ++i){
            let j
            setTimeout(()=>{
                for (j = i - 1;(j >= 0 && barsArray[i].height < barsArray[j].height);--j){
                    setTimeout(()=>{
                        swapAnimation(barsArray[j], barsArray[j+1])
                    }, (i-j) * (1000/speed));
                }
            }, (i-1) * (length) * (1000/speed));
        }
    }
